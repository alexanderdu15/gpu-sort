...
void runTestForType(const string& type_name, uint32_t* sizes, int RUNS_PER_SIZE,
                   SortMethod* methods, int num_methods, const int* block_sizes, 
                   int num_block_sizes, ofstream& outfile) {
    for (int m = 0; m < num_methods; m++) {
        ...
        for (int b = 0; b < num_block_sizes; b++) {
            ...
            cout << "\nTesting " << type_name << " with " << method_name 
                 << " sort (Block Size: " << block_size << ")" << endl;
            ...
            for (int s = 0; s < 16; s++) {
                ...
                for (int run = 0; run < RUNS_PER_SIZE; run++) {
                    GPUSorter<T> sorter(size, method, block_size);
                    ...
                    outfile << type_name << ","
                           << method_name << ","
                           << size << ","
                           << block_size << ","  // Add block size to output
                           << run + 1 << ","
...
int main() {
    ...
    SortMethod methods[] = {
        ...
        // SortMethod::SINGLE_THREAD, // too slow to test lol
    };
    ...
    outfile << "Type,Method,Size,BlockSize,Run,Correct,Total Time (ms),Kernel Time (ms)" << endl;

    const int block_sizes[] = {
        64,     // 2 warps
        128,    // 4 warps
        256,    // 8 warps  (current default)
        512,    // 16 warps
        1024    // 32 warps (maximum)
    };
    ...
    //runTestForType<uint32_t>("uint32", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
    runTestForType<int32_t>("int32", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
    //runTestForType<uint64_t>("uint64", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
    //runTestForType<int64_t>("int64", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
    //runTestForType<float>("float", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
    //runTestForType<double>("double", sizes, RUNS_PER_SIZE, methods, NUM_METHODS, block_sizes, NUM_BLOCK_SIZES, outfile);
